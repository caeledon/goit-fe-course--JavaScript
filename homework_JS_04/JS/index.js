...
const products = {
  ...
  /* 
    ...
    Поля будущего объекта кассира (🔔 объявляются как this.имя_поля в конструкторе): 
      ...
      - countTotalPrice(order) - метод, получает объект список покупок, считает общую стоимость покупок.
  ...
  */
  ...
  const batanta = new Cashier('Batanta', products);
  
  // Проверяем исходные значения полей
  // console.log(mango.name); // Mango
  console.log(batanta.name);//Batanta
  // console.log(mango.productDatabase); // ссылка на базу данных продуктов (объект products)
  console.log(batanta.productDatabase);//{bread: 10, milk: 15, apples: 20, chicken: 50, cheese: 40}
  // console.log(mango.customerMoney); // 0
  console.log(batanta.customerMoney); // 0
  
  // Вызываем метод countTotalPrice для подсчета общей суммы
  // передавая order - список покупок пользователя
  // const totalPrice = mango.countTotalPrice(order);
  const totalPrice = batanta.countTotalPrice(order);
  ...
  batanta.getCustomerMoney(245);
  
  // Проверяем что в поле с деньгами пользователя
  // console.log(mango.customerMoney); // 300
  console.log(batanta.customerMoney); // 245
  
  // Вызываем countChange для подсчета сдачи
  // const change = mango.countChange(110);
  const change = batanta.countChange(110);
  
  // Проверяем что нам вернул countChange
  // console.log(change); // 190
  console.log(change);//135
  ...
  if(change !== null) {
    // При успешном обслуживании вызываем метод onSuccess
   batanta.onSuccess(change); // Спасибо за покупку, ваша сдача 135
 } else {
   // При неудачном обслуживании вызываем метод onError   
   batanta.onError(); // Очень жаль, вам не хватает денег на покупки
 }
